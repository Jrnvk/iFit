 ...
 || (length(pars)^2*output.fevalDuration < 60 ... % should spend less than a minute to compute the Hessian
  ...
  try
    [dp, covp, corp,jac,hessian]  = ...
      inline_estimate_uncertainty(fun, pars, pars_all, options, constraints, varargin{:});
    if ~isempty(covp)
...
% This is controlled from fmin_private_wrapper
  ...
  case {'fminlm2','LMFnlsq2'}
  % Levenberg-Maquardt steepest descent version 2013------------------------------------------
    % LMFnlsq2 minimizes the sum of the squares of the inline_objective: sum(inline_objective.^2)
    yresul=inline_objective(fun, pars, varargin{:});
    if (size(yresul,2)==size(options.TolFun,2)),
      options.TolFun=sum(options.TolFun')';
    end;
    fprintf('size of function return vector: [%d %d\n]',size(yresul));
    [pars, fval, iterations,res,XY] = LMFnlsq2(@(pars) inline_objective(fun, pars, varargin{:}), pars, ...
             'Display',[0,0], 'FunTol', options.TolFun, 'XTol', options.TolX, ...
             'MaxIter', options.MaxIter,'Jacobian',options.Jacobian, ...
             'Trace',options.Trace,'ScaleD',options.ScaleD,...
             'Lambda',options.Lambda,'Printf',options.Printf,  ...
             'Basdx',options.Basdx);
